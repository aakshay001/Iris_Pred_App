...
def prediction(SepalLengthCm,SepalWidthCm,PetalLengthCm,PetalWidthCm):
	species = svc_model.predict([[SepalLengthCm,SepalWidthCm,PetalLengthCm,PetalWidthCm]])
	species = species[0]
...
s_l = st.slider("Sepal Length",0.0,10.0)
s_w = st.slider("Sepal Width",0.0,10.0)
p_l = st.slider("Petal Length",0.0,10.0)
p_w = st.slider("Petal Width",0.0,10.0)
if st.button("Predict"):
	species_type = prediction(s_l, s_w, p_l, p_w)
